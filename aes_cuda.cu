...
// 预计算的 T-box 表
__constant__ unsigned int d_T0[256] = {
...
__constant__ unsigned int d_T1[256] = {
...
__constant__ unsigned int d_T2[256] = {
...
__constant__ unsigned int d_T3[256] = {
...
// 密钥扩展
__device__ void KeyExpansion(unsigned char* roundKey, const unsigned char* key) {
  int i,j;
...
// 有限域乘法
__device__ inline unsigned char gmul(unsigned char a, unsigned char b) {
  unsigned char p = 0;
  for(int i=0; i<8; i++) {
    if(b & 1) p ^= a;
...
// 轮密钥加
__device__ void AddRoundKey(unsigned char* state, const unsigned char* roundKey, int round) {
  for(int i=0; i<16; i++) {
...
// // AES加密核心
// __device__ void AES_Encrypt(unsigned char* state, const unsigned char* expandedKey) {
...
// 加密核函数
// __global__ void AES_Kernel(const unsigned char* input, 
...
__global__ void AES_Kernel(const unsigned char* input, 
                          ...
                          const unsigned char* key,
                          int numBlocks,
                          int blocksPerThread) {  // 每个线程块处理的数据块数
    __shared__ unsigned int s_T0[256], s_T1[256], s_T2[256], s_T3[256];
    ...
    state[2] ^= s_roundKey[2];
    state[3] ^= s_roundKey[3];

    // 前9轮T-box变换
    for(int round = 1; round < 10; round++){
        unsigned int t0, t1, t2, t3;
        t0 = s_T0[(state[0] >> 24) & 0xff] ^
             s_T1[(state[1] >> 16) & 0xff] ^
             s_T2[(state[2] >>  8) & 0xff] ^
             s_T3[(state[3])       & 0xff] ^
             s_roundKey[round*4 + 0];
        t1 = s_T0[(state[1] >> 24) & 0xff] ^
             s_T1[(state[2] >> 16) & 0xff] ^
             s_T2[(state[3] >>  8) & 0xff] ^
             s_T3[(state[0])       & 0xff] ^
             s_roundKey[round*4 + 1];
        t2 = s_T0[(state[2] >> 24) & 0xff] ^
             s_T1[(state[3] >> 16) & 0xff] ^
             s_T2[(state[0] >>  8) & 0xff] ^
             s_T3[(state[1])       & 0xff] ^
             s_roundKey[round*4 + 2];
        t3 = s_T0[(state[3] >> 24) & 0xff] ^
             s_T1[(state[0] >> 16) & 0xff] ^
             s_T2[(state[1] >>  8) & 0xff] ^
             s_T3[(state[2])       & 0xff] ^
             s_roundKey[round*4 + 3];
        state[0] = t0; state[1] = t1; state[2] = t2; state[3] = t3;
    }

    // 最后一轮：只作S-box替换 + 轮密钥加
    unsigned int out[4];
    out[0] = (d_sbox[(state[0] >> 24) & 0xff] << 24) |
             (d_sbox[(state[1] >> 16) & 0xff] << 16) |
             (d_sbox[(state[2] >>  8) & 0xff] <<  8) |
             (d_sbox[(state[3])       & 0xff]);
    out[1] = (d_sbox[(state[1] >> 24) & 0xff] << 24) |
             (d_sbox[(state[2] >> 16) & 0xff] << 16) |
             (d_sbox[(state[3] >>  8) & 0xff] <<  8) |
             (d_sbox[(state[0])       & 0xff]);
    out[2] = (d_sbox[(state[2] >> 24) & 0xff] << 24) |
             (d_sbox[(state[3] >> 16) & 0xff] << 16) |
             (d_sbox[(state[0] >>  8) & 0xff] <<  8) |
             (d_sbox[(state[1])       & 0xff]);
    out[3] = (d_sbox[(state[3] >> 24) & 0xff] << 24) |
             (d_sbox[(state[0] >> 16) & 0xff] << 16) |
             (d_sbox[(state[1] >>  8) & 0xff] <<  8) |
             (d_sbox[(state[2])       & 0xff]);
    ...
    // 写回
    output[base+0]  = (out[0] >> 24) & 0xff;
    output[base+1]  = (out[0] >> 16) & 0xff;
    output[base+2]  = (out[0] >>  8) & 0xff;
    output[base+3]  = (out[0])       & 0xff;
    output[base+4]  = (out[1] >> 24) & 0xff;
    output[base+5]  = (out[1] >> 16) & 0xff;
    output[base+6]  = (out[1] >>  8) & 0xff;
    output[base+7]  = (out[1])       & 0xff;
    output[base+8]  = (out[2] >> 24) & 0xff;
    output[base+9]  = (out[2] >> 16) & 0xff;
    output[base+10] = (out[2] >>  8) & 0xff;
    output[base+11] = (out[2])       & 0xff;
    output[base+12] = (out[3] >> 24) & 0xff;
    output[base+13]  = (out[3] >> 16) & 0xff;
    output[base+14] = (out[3] >>  8) & 0xff;
    output[base+15] = (out[3])       & 0xff;
  }
...
int main() {
    const int dataSize = 1024*1024*1024; // 1024M数据
    unsigned char *plaintext = new unsigned char[dataSize];
    for (int i = 0; i < dataSize; ++i) {
        plaintext[i] = static_cast<unsigned char>(rand() % 256); // 随机生成数据
    }
    ...
    // 计算PKCS#7填充
    int pad = 16 - (len % 16);
    pad = pad == 0 ? 16 : pad;  // 若长度正好为块大小倍数，填充完整块
    int padded_len = len + pad;
    int numBlocks = padded_len / 16;
    
    // int num_blocks = 8192*8192; // 64M数据
    // padded_len = num_blocks * 16;

    // 分配主机内存并填充数据
    unsigned char *h_input = new unsigned char[padded_len];
    unsigned char *h_output = new unsigned char[padded_len];
    unsigned char h_key[16] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
                               0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};

    // 填充数据
    memcpy(h_input, plaintext, len);
    for (int i = len; i < padded_len; ++i) {
        h_input[i] = static_cast<unsigned char>(pad);
    }

    // 分配设备内存
    unsigned char *d_input, *d_output, *d_key;
    cudaMalloc(&d_input, padded_len);
    cudaMalloc(&d_output, padded_len);
    cudaMalloc(&d_key, 16);

    // 数据拷贝到设备
    cudaMemcpy(d_input, h_input, padded_len, cudaMemcpyHostToDevice);
    cudaMemcpy(d_key, h_key, 16, cudaMemcpyHostToDevice);
    ...
    AES_Kernel<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, d_key, numBlocks, blocksPerThread);

    cudaEventRecord(stop);  // 记录结束时间
    cudaEventSynchronize(stop); // 等待核函数执行完成

    float milliseconds = 0;
    cudaEventElapsedTime(&milliseconds, start, stop); // 计算时间差（毫秒）
