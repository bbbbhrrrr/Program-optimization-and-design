...
const int numBlocks=1024*1024*512;
...
__constant__ uint4 d_roundKeys[11];

// 修复后的密钥扩展函数
__host__ void KeyExpansion(uint4* roundKeys, const unsigned char* key) {
  unsigned char expKey[176] = {0}; 
  ...
  for(int i=4; i<44; i++) {
      ...
      for(int j=0; j<4; j++)
          expKey[i*4 + j] = expKey[(i-4)*4 + j] ^ temp[j];
  }
  
  // 转换为uint4格式
  for(int i=0; i<11; i++) {
      roundKeys[i].x = __builtin_bswap32(*reinterpret_cast<uint32_t*>(expKey + i*16));
	  roundKeys[i].y = __builtin_bswap32(*reinterpret_cast<uint32_t*>(expKey + i*16 + 4));
	  roundKeys[i].z = __builtin_bswap32(*reinterpret_cast<uint32_t*>(expKey + i*16 + 8));
	  roundKeys[i].w = __builtin_bswap32(*reinterpret_cast<uint32_t*>(expKey + i*16 + 12));
  }
}
...
int main() {
    ...
    // 生成测试数据时使用uint4结构
    uint4* h_input = new uint4[numBlocks];
    uint4* h_output = new uint4[numBlocks];
	for (int i = 0; i < numBlocks; ++i) {
		h_input[i].x = __builtin_bswap32(rand());
		h_input[i].y = __builtin_bswap32(rand());
		h_input[i].z = __builtin_bswap32(rand());
		h_input[i].w = __builtin_bswap32(rand());
	}
	std::cout << "明文（Hex）:\n";
	for (int i = numBlocks - 16; i < numBlocks; ++i) {
		printf("Block %d: %08x %08x %08x %08x\n", i, h_input[i].x, h_input[i].y, h_input[i].z, h_input[i].w);
	}


    // 设备内存分配
    uint4 *d_input, *d_output;
    cudaMalloc(&d_input, numBlocks * sizeof(uint4));
    cudaMalloc(&d_output, numBlocks * sizeof(uint4));
	cudaMemcpy(d_input, h_input, numBlocks * sizeof(uint4), cudaMemcpyHostToDevice);
	cudaMemcpy(h_output, d_output, numBlocks * sizeof(uint4), cudaMemcpyDeviceToHost);

    unsigned char h_key[16] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
      0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};

    uint4 h_roundKeys[11];
    KeyExpansion(h_roundKeys, h_key);

       // 拷贝轮密钥到设备
       cudaMemcpyToSymbol(d_roundKeys, h_roundKeys, 11 * sizeof(uint4));


    // 启动核函数配置
    int threadsPerBlock = 1024; // 每个线程块的线程数量
    int blocksPerThread = 1048576;  // 每个线程处理的数据块数量
	int blocksPerGrid = (numBlocks + blocksPerThread - 1) / blocksPerThread;
    ...
    AES_Kernel<<<blocksPerGrid, threadsPerBlock>>>(d_input, d_output, numBlocks, blocksPerThread);
    ...
    // 计算内存吞吐量
    float memory_throughput = numBlocks * sizeof(uint4) * 2 / milliseconds / 1e6; // GB/s
